...
def averageRoundsJsons(j):
    ...
    avgIpv6sentPackets = sum([j[i]["results"]["ipv6sumSentPackets"] for i in range(0, len(j))])/len(j)
    avgIpv6receivedPackets = sum([j[i]["results"]["ipv6sumReceivedPackets"] for i in range(0, len(j))])/len(j)
    ...
    numDatapoints = len(j)
    return {"avgLostPackets":avgNumLostPkts, "avgLossPercent":avgLossPercent, "avgSendRate":avgSendRate, "avgReceiveRate":avgReceiveRate, "avgTimeDiffSecs":avgTimeDiffSecs, "avgSumCacheHits":avgSumCacheHits, "numDatapoints":numDatapoints, "avgNumSentPkts":avgNumSentPkts, "avgNumForwards":avgNumForwards, "avgL2sentPackets": avgl2sentPackets, "avgL2receivedPackets":avgl2receivedPackets, "avgIpv6sentPackets": avgIpv6sentPackets, "avgIpv6receivedPackets":avgIpv6receivedPackets}
...
async def cachingExperiment(delayus=10000, payloadsizebytes=32, transfersizebytes=1024, rounds=1, mode=3, cache=1, code=1, numcacheblocks=16, resultsDir="./"):
    ...
    outFilenamePrefix = f"m_{mode}_cache{cache}_code{code}_numcache{numcacheblocks}_delay{delayus}_pl{payloadsizebytes}_tx{transfersizebytes}_routers{len(devices['routers'])}_jammers{len(devices['jammers'])}"
    overallJson = []
    ...
    for round in range(0, rounds):
        ...
        rxOut += comm.sendSerialCommand(rxDev, f"iperf config mode {mode} delayus {delayus} plsize {payloadsizebytes} xfer {transfersizebytes} cache {cache}", cooldownS=3)
        txOut += comm.sendSerialCommand(txDev, f"iperf config mode {mode} delayus {delayus} plsize {payloadsizebytes} xfer {transfersizebytes} cache {cache}", cooldownS=3)
        ...
        futures = []
        for r in devices["routers"]:
            future = sendCmdBackground(r, f"iperf config numcacheblocks {numcacheblocks} cache {cache} code {code} delayus {delayus}")
            futures.append(future)
        time.sleep(1)
        await asyncio.gather(*futures)
